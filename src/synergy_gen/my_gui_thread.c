...
static uint8_t my_gui_thread_stack[8192] BSP_PLACE_IN_SECTION_V2(".stack.my_gui_thread") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
void my_gui_thread_create(void)
{
    ...
    err = tx_thread_create (&my_gui_thread, (CHAR *) "My Gui Thread", my_gui_thread_func, (ULONG) NULL,
                            &my_gui_thread_stack, 8192, 20, 20, 2, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
